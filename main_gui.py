"""
main_gui.py – Görselleştirme ve Arayüz (Frontend)
==================================================
...
# Teknik terimlerin kullanıcı dostu Türkçe karşılıkları
...
class BobPanel(QWidget):
    ...
    def show_next_step(self) -> bool:
        ...
        if self._current_step >= len(self._steps):
            is_valid = step.data.get("verification_result", False)
            if is_valid:
                self._result_label.setText(
                    "✅ DOĞRULAMA BAŞARILI\n\n"
                ...
                self._result_label.setStyleSheet(
                    f"color: {COLORS['accent_green']}; font-size: 14px;"
                )
            else:
                self._result_label.setText(
                    "❌ DOĞRULAMA BAŞARISIZ\n\n"
                    "İmza doğrulanamadı — mesaj değiştirilmiş "
                    "veya gönderici kimliği sahte olabilir!"
                )
                self._result_label.setStyleSheet(
                    f"color: {COLORS['accent_red']}; font-size: 14px;"
                )
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self) -> None:
        ...
        main_layout.addWidget(self._key_info_group)
        ...
        cmp_cols = QHBoxLayout()
        cmp_cols.setSpacing(12)

        # Alice sütunu
        alice_cmp = QFrame()
        alice_cmp.setStyleSheet(
            f"QFrame {{ background-color: {COLORS['bg_card']}; border-radius: 8px; }}"
        )
        alice_cmp_inner = QVBoxLayout(alice_cmp)
        alice_cmp_inner.setContentsMargins(10, 8, 10, 8)
        alice_cmp_title = QLabel("👩\u200d💻 Alice'in Gönderdiği")
        alice_cmp_title.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        alice_cmp_title.setStyleSheet(f"color: {COLORS['accent_blue']};")
        alice_cmp_inner.addWidget(alice_cmp_title)
        self._alice_msg_cmp = QLabel("")
        self._alice_msg_cmp.setWordWrap(True)
        self._alice_msg_cmp.setStyleSheet(
            f"color: {COLORS['text_secondary']}; font-size: 12px;"
        )
        self._alice_msg_cmp.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse
        )
        alice_cmp_inner.addWidget(self._alice_msg_cmp)
        self._alice_hash_cmp = QLabel("")
        self._alice_hash_cmp.setWordWrap(True)
        self._alice_hash_cmp.setStyleSheet(
            f"color: {COLORS['text_muted']}; font-size: 11px; font-family: monospace;"
        )
        self._alice_hash_cmp.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse
        )
        alice_cmp_inner.addWidget(self._alice_hash_cmp)
        cmp_cols.addWidget(alice_cmp, stretch=3)

        # Orta: eşleşme göstergeleri
        mid_frame = QFrame()
        mid_frame.setStyleSheet(
            f"QFrame {{ background-color: {COLORS['bg_card']}; border-radius: 8px; }}"
        )
        mid_inner = QVBoxLayout(mid_frame)
        mid_inner.setAlignment(Qt.AlignmentFlag.AlignCenter)
        mid_inner.setSpacing(4)

        self._cmp_msg_icon = QLabel("")
        self._cmp_msg_icon.setFont(QFont("Segoe UI", 26, QFont.Weight.Bold))
        self._cmp_msg_icon.setAlignment(Qt.AlignmentFlag.AlignCenter)
        mid_inner.addWidget(self._cmp_msg_icon)
        ...
        mid_inner.addWidget(self._cmp_msg_label)

        sep = QLabel("─────")
        sep.setAlignment(Qt.AlignmentFlag.AlignCenter)
        sep.setStyleSheet(f"color: {COLORS['border']};")
        mid_inner.addWidget(sep)

        self._cmp_hash_icon = QLabel("")
        self._cmp_hash_icon.setFont(QFont("Segoe UI", 26, QFont.Weight.Bold))
        self._cmp_hash_icon.setAlignment(Qt.AlignmentFlag.AlignCenter)
        mid_inner.addWidget(self._cmp_hash_icon)
        ...
        mid_inner.addWidget(self._cmp_hash_label)
        cmp_cols.addWidget(mid_frame, stretch=1)

        # Bob sütunu
        bob_cmp = QFrame()
        bob_cmp.setStyleSheet(
            f"QFrame {{ background-color: {COLORS['bg_card']}; border-radius: 8px; }}"
        )
        bob_cmp_inner = QVBoxLayout(bob_cmp)
        bob_cmp_inner.setContentsMargins(10, 8, 10, 8)
        bob_cmp_title = QLabel("👨\u200d💻 Bob'un Aldığı")
        bob_cmp_title.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        bob_cmp_title.setStyleSheet(f"color: {COLORS['accent_green']};")
        bob_cmp_inner.addWidget(bob_cmp_title)
        self._bob_msg_cmp = QLabel("")
        self._bob_msg_cmp.setWordWrap(True)
        self._bob_msg_cmp.setStyleSheet(
            f"color: {COLORS['text_secondary']}; font-size: 12px;"
        )
        self._bob_msg_cmp.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse
        )
        bob_cmp_inner.addWidget(self._bob_msg_cmp)
        ...
        bob_cmp_inner.addWidget(self._bob_hash_cmp)
        cmp_cols.addWidget(bob_cmp, stretch=3)

        cmp_outer.addLayout(cmp_cols)
        main_layout.addWidget(self._comparison_group)
    ...
    def _on_keygen(self) -> None:
        """RSA anahtar çiftlerini üretir ve sonuçları şık biçimde gösterir."""
        alice_keys, bob_keys = self._crypto.setup_keys()

        # PEM base64 gövdesini al (başlık/bitiş satırları hariç)
    ...
    def _on_next_step(self) -> None:
        ...
        if self._phase == "alice":
            ...
            if self._packet is not None:
                ...
                try:
                    message, is_valid, bob_steps = self._crypto.bob_receive(
                        self._packet
                    )
    ...
    def _show_comparison(self, orig: str, received: str) -> None:
        """Alice'in gönderdiği ile Bob'un aldığı mesajı karşılaştırmalı gösterir."""
        ...
        # Alice sütunu
        msg_preview = orig if len(orig) <= 100 else orig[:100] + "…"
        self._alice_msg_cmp.setText(f"📝 Mesaj:\n{msg_preview}")
        self._alice_hash_cmp.setText(
            f"\n🔷 SHA-256 Özeti:\n{orig_hash[:32]}\n{orig_hash[32:]}"
        )

        # Bob sütunu
        recv_preview = received if len(received) <= 100 else received[:100] + "…"
        self._bob_msg_cmp.setText(f"📝 Mesaj:\n{recv_preview}")
        self._bob_hash_cmp.setText(
            f"\n🔷 SHA-256 Özeti:\n{recv_hash[:32]}\n{recv_hash[32:]}"
        )
